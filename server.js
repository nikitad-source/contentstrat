...
app.post("/api/buckets", async (req, res) => {
  try {
    ...
    if (specialDays && specialDays.trim() !== "") {
      user += `\n\nCRITICAL: One of the buckets MUST be exactly named "Special Days" to cover these occasions: ${specialDays}.`;
    }
...
app.post("/api/posts", async (req, res) => {
  try {
    const { brand, platforms, objective, research, personas, bucket, globalFeedback, bucketFeedback, specialDays, postCountPerBucket } = req.body;
    const pCount = parseInt(postCountPerBucket) || 5;
    const platformPosts = {};
      ...
      const schema = {
        ...
        items: {
          ...
          properties: {
            ...
            visual_cue: { type: "STRING", description: "Detailed description for the designer" },
            inner_copy: { type: "STRING", description: "Short, punchy text that goes INSIDE the image/video" },
            outer_copy: { type: "STRING", description: "The main caption for the post" },
            reference_link: { type: "STRING", description: "A highly targeted Pinterest search URL for this specific visual style" }
          },
          required: ["format", "idea", "visual_cue", "inner_copy", "outer_copy", "reference_link"]
        }
      };
